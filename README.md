...
- 现有定制类平台普遍**缺少社区氛围**、内容治理能力与交易过程留痕。

本平台面向上述痛点，构建一个集 **作品展示、定制需求发布、报价沟通、订单跟踪、社区互动、内容治理** 于一体的文创手作交易平台，促进文创产业数字化转型。
...
│   │   ├── 02_business_core.sql                  # 创作者/分类/商品（含审核字段）/需求/报价/订单/支付
│   │   ├── 03_social_interaction.sql             # 作品/评论/互动/聊天/敏感词词库
│   │   ├── 04_system_menus.sql                   # 菜单 + 角色 + 权限（含数据看板/敏感词/审核）
│   │   └── 05_test_data.sql                      # 演示数据（可选）
│   ├── dashboard_menu_upgrade_20260501.sql       # 旧库追加"数据看板"菜单
│   ├── sensitive_word_upgrade_20260501.sql       # 旧库追加敏感词词库表 + 菜单
│   ├── product_comment_audit_upgrade_20260501.sql# 旧库追加商品审核字段 + 审核按钮权限
│   └── README.md                                 # 数据库安装指南
├── doc/
...
| `creative_creator` | 创作者档案（绑定系统用户、店铺名、等级；含审核状态） |
| `creative_product` | 现货/定制商品（含 `audit_status` 审核状态） |
...
| `creative_comment` | 旧版评论表（作品评论；含 `audit_status`，后台审核流走这张） |
| `creative_interaction_comment` | 通用评论表（前台社区评论统一走这张，按 `target_type/target_id` 区分商品/作品） |
| `creative_favorite_follow` | 收藏与关注（支持商品/创作者/作品三种 targetType） |
| `creative_chat_session` | 买家 ↔ 创作者聊天会话（按 target 业务对象唯一） |
| `creative_chat_message` | 聊天消息（text / image，含已读状态） |
| `creative_sensitive_word` | 敏感词词库（DFA 算法的数据来源，CRUD 后热更新） |
...
1. **动态敏感词过滤机制（论文核心算法亮点，已落地）**：词库表 `creative_sensitive_word` 由后台 CRUD 维护；使用 **DFA（Deterministic Finite Automaton）** 算法进行多模式匹配（构建 O(L)、匹配 O(N\*K)），性能优于正则；用 `AtomicReference<Node>` 维护字符树，CRUD 后内部 reload 实现热更新；接入评论提交（旧版 + 通用两张表）和聊天文本入口（仅 text 类型），命中即抛 `ServiceException`。
...
**已经跑过 04 的旧库**按需追加 2026-05-01 的升级补丁（每个补丁都是幂等的）：

```sql
source sql/dashboard_menu_upgrade_20260501.sql;        -- 数据看板顶级菜单
source sql/sensitive_word_upgrade_20260501.sql;        -- 敏感词词库 + 后台菜单
source sql/product_comment_audit_upgrade_20260501.sql; -- 商品审核字段 + 商品/评论审核按钮
```

执行后管理员重新登录刷新菜单即可看到新增的"数据看板"、"敏感词库"以及商品/评论的"通过/驳回"按钮。新库已合并到 `01-04` 安装脚本，**不要重复执行**。
...
| 阶段 6：管理后台增强（数据看板 ✅ / 敏感词过滤 ✅ / 商品评论审核流 ✅） | ✅ 已完成 |
